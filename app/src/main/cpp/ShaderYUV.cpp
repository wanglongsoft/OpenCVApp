...
GLuint ShaderYUV::LoadProgram(const char *vShaderStr, const char *fShaderStr) {
    ...
    gl_change_type = glGetUniformLocation(mProgram,"vChangeType");
    LOGD("LoadProgram out gl_change_type : %d", gl_change_type);

    gl_split_screen_type = glGetUniformLocation(mProgram,"vSplitScreen");
    LOGD("LoadProgram out gl_split_screen_type : %d", gl_split_screen_type);
...
void ShaderYUV::Render(uint8_t *data[]) {
    //激活第一层纹理，绑定到创建的纹理
    glUniform1i(gl_change_type, change_type);
    glUniform1i(gl_split_screen_type, split_screen_type);
    y_data = data[0];
    u_data = data[1];
    v_data = data[2];
...
void ShaderYUV::blackWhiteRender() {
    LOGD("blackWhiteRender in");
    change_type = 3;
    LOGD("blackWhiteRender out");
}

void ShaderYUV::warmColorRender() {
    LOGD("warmColorRender in");
    change_type = 1;
    LOGD("warmColorRender out");
}

void ShaderYUV::normalColorRender() {
    LOGD("normalColorRender in");
    change_type = 0;
    split_screen_type = 0;
    LOGD("normalColorRender out");
}

void ShaderYUV::coolColorRender() {
    LOGD("coolColorRender in");
    change_type = 2;
    LOGD("coolColorRender out");
}

void ShaderYUV::splitScreenRender() {
    LOGD("normalColorRender in");
    split_screen_type = 1;
    LOGD("normalColorRender out");
}
